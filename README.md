...
这是一个使用 React、Vite 与 IndexedDB 构建的本地优先奢侈零售概念网站。当前目录包含「粤凳系列 / GUANGDONG STOOL SERIES」与「煲点什么 / WHAT'S SIMMERING」；粤凳系列收录无靠背方凳「粤凳 01 / GUANGDONG STOOL N°01」、带弧形靠背与一体扶手的「粤凳 02 / GUANGDONG STOOL N°02」，以及三色档案套装。

「粤凳 02」以浅蓝为默认版本，另提供白、深蓝、黄、红、橙五种常设色彩。六款共用 ¥18,800 定价与 EDITION 会籍购买政策，每单最多四件；默认画廊同时展示六色堆叠场景与虚构成年米兰模特造型。

「慢慢滚 / SIMMER DOWN」是一款单柄平底浅煲，仅提供砂褐变体，定价 ¥18,800，目录状态为 `standard / active`。商品采用 EDITION 会籍购买政策，每单最多四件；六张画廊影像包括四个商品视角、一张无人物白色展馆场景，以及一张虚构成年米兰模特轻触短柄的造型场景。两张场景图均明确标为品牌 AI 概念影像。新增商品由目录自动进入商品总览、所属系列、搜索和新品区域，不替换首页主视觉。
