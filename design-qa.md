...
- 正式目录包含 1 个系列、3 个独立商品、10 个稳定变体、39 个唯一资源；测试 fixture 另覆盖第二系列与独立商品，但不会进入正式目录。
...
| 视口 | 结果 |
| --- | --- |
| 1440 × 1024 | 商品总览、系列页与粤凳 02 详情无横向溢出；桌面导航、六色选择器、购买按钮和画廊正常 |
| 1024 × 768 | 中型布局无横向溢出；商品标题、六色选择器、购买按钮与图片均可达且无遮挡 |
| 390 × 844 | 移动导航、单列目录与横向画廊正常；标题和首图无遮挡、越界或横向滚动 |
...
- `npm run catalog:validate`：1 个系列、3 个商品、10 个变体、39 个资源通过。
- Vitest：4 个测试文件，57 项测试全部通过。
- Vite 生产构建：通过。
